  ...
  AttachValidationListeners(inputsDefinition) {
    $(inputsDefinition).map((index, inputElement) => {
      ...
      input.on("blur change", (event) => {
        ...
        let patrents = $(event.target).parents(".input-container");
        let inputContainer = $(event.target).parents(".input-container").first();
        let requiredAlertIcon = inputContainer.find(".required-alert");
        let validFieldIcon = inputContainer.find(".valid-field-info");
        let errorFieldIcon = inputContainer.find(".error-field-info");
        if (value.trim() != "") {
